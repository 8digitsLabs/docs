...
<Note>
  The capture is on-chain and measured. The split is treasury policy rather than something the contracts enforce: the hook pays its earnings to a single treasury address, and the allocation happens from there. OKA itself is burnable by design, so a buyback contract can retire it on the treasury's behalf without ever taking custody.
</Note>
