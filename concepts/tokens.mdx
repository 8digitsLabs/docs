...
It also means **every OKA trade routes through the index pool**. Buying OKA with a stablecoin is two hops: stable to IOKA across the band, then IOKA to OKA. The first hop pays the band's spread, which the fund keeps.

<Note>
  That routing is not theoretical. On the test deployment, a single transaction bought IOKA with the stablecoin on the hook's pool and swapped it for OKA on the second, paying the band's spread on the way through.
</Note>
