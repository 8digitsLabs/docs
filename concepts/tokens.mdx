...
<Note>
  Voting is not implemented yet. The vault's basket functions are owner-controlled in the deployed contracts, and the governance layer that hands those rights to OKA holders is still to be built.
</Note>
...
<Note>
  The rules below are not final, and they will be restated before the airdrop settles.
</Note>
